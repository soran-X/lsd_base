...
/**
 * CompanyComboboxController
 *
 * Single-select search combobox for picking an existing company.
 * Does NOT allow creating new companies.
 *
 * Values:
 *   url – search endpoint, e.g. "/companies/search.json"
 *
 * Targets:
 *   input       – visible text input for searching / showing selection
 *   hiddenField – hidden input that holds the selected company ID
...
export default class extends Controller {
  ...
  async _fetch(q) {
    ...
    try {
      ...
      if (e.name !== "AbortError") console.error("Company search error:", e)
    }
  }
  ...
  _render(query) {
    if (!this._results.length) {
      this.dropdownTarget.innerHTML =
        '<p class="px-4 py-3 text-sm text-gray-400 italic">No companies found.</p>'
      this._open()
      return
    }
