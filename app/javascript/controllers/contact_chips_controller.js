...
/**
 * ContactChipsController
 *
 * Multi-select search combobox for picking existing contacts (chip tags).
 *
 * Values:
 *   search-url  – URL for GET search endpoint (returns [{id, label}])
 *   field-name  – Hidden input name, e.g. "book[agent_contact_ids][]"
 */
export default class extends Controller {
  ...
  connect() {
    ...
    this.chipsTarget.querySelectorAll("[data-contact-id]").forEach(chip => {
      this.selected.set(parseInt(chip.dataset.contactId), chip.dataset.contactLabel)
    })
  }
  ...
  removeChip(event) {
    ...
    const chip = this.chipsTarget.querySelector(`[data-contact-id="${id}"]`)
  ...
  async _search(query) {
    ...
    try {
      ...
      if (e.name !== "AbortError") console.error("Contact chips search error:", e)
    }
  }
  ...
  _renderDropdown(query) {
    if (!this.results.length) {
      this.listTarget.innerHTML =
        '<li class="px-4 py-3 text-sm text-gray-400 italic">No contacts found.</li>'
      this._open()
      return
    }
    ...
    this.listTarget.innerHTML = this.results.map((c, i) => `
      <li role="option"
          ...
          data-action="click->contact-chips#selectResult mouseenter->contact-chips#highlight"
  ...
  _select(contact) {
    if (this.selected.has(contact.id)) return
    this.selected.set(contact.id, contact.label)
    this._addChip(contact)
    this.inputTarget.value = ""
    this._close()
  ...
  _addChip(contact) {
    ...
    chip.dataset.contactLabel = contact.label
    chip.innerHTML = `
      <span>${contact.label}</span>
      <input type="hidden" name="${this.fieldNameValue}" value="${contact.id}">
      <button type="button"
